...
const FIELDS: { key: keyof Params; label: string; hint: string; kind: Kind; group: string }[] = [
  ...
  { key: 'coin_up_exp', label: '코인 상승폭 지수', hint: '기본 3. 낮추면 큰 상승이 잦아져요.', kind: 'float', group: 'coin' },
  { key: 'coin_down_exp', label: '코인 하락폭 지수', hint: '기본 2. 높이면 소폭 하락에 몰려요.', kind: 'float', group: 'coin' },
  ...
  { key: 'coin_price_cap', label: '코인 가격 상한(원)', hint: '비우면 상한 없음. 권장 5000000', kind: 'nullableInt', group: 'coin' },
  { key: 'stock_sensitivity', label: '주식 감도 계수', hint: '소수. 0.3 = 쏠림 1배당 ±30%', kind: 'float', group: 'stock' },
  { key: 'stock_min_price', label: '주식 최저가(원)', hint: '기본 1000', kind: 'int', group: 'stock' },
  { key: 'virtual_liquidity', label: '가상 유동성 L(원)', hint: '종목마다 매수금액에 더해요. 예: 5000000', kind: 'int', group: 'stock' },
  { key: 'daily_buy_limit_ratio', label: '1일 1종목 매수 상한', hint: '소수. 0.4 = 총자산의 40%', kind: 'float', group: 'trade' },
  { key: 'reward_coin_quantity', label: '인증 보상(개)', hint: '승인 1건당 지급하는 병더리움 개수', kind: 'int', group: 'trade' },
...
export function ParamsTab() {
  ...
  return (
    <form onSubmit={save} noValidate>
      <Stack gap={5}>
        {saveError && <Alert tone="danger" title="파라미터를 저장하지 못했어요">{saveError}</Alert>}
        {GROUPS.map((g) => (
